...
import { KosmicRaffle, IDL } from "../target/types/kosmic_dao_raffle";
import { airdrop, getAtaForMint, getRawTokenAccount, log, mintNFT } from "./utils";
import idl from '../target/idl/kosmic_dao_raffle.json';
import { ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID } from "@solana/spl-token";
import { PublicKey, SYSVAR_RENT_PUBKEY } from "@solana/web3.js";

const programId = '6hQVmZSoUjWdBniVExRorWJxCDFmJwznXis2HSopBdu6'

describe("kosmic-dao-raffle", () => {
  // Configure the client to use the local cluster.
  anchor.setProvider(anchor.AnchorProvider.env());
  const program =  new Program<KosmicRaffle>(IDL, new anchor.web3.PublicKey(programId));
